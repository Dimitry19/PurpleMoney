package cm.purplemoney.constants;

public class PortalConstants {
	
	public static final String DEFAULT_COMPANY = "001";
	public static final String DEFAULT_ENVIRONMENT = "001";
	public static final String DEFAULT_WAREHOUSE = "001";
	public static final String GREEN = "GREEN";
	public static final String RED = "RED";
	public static final String ORANGE = "ORANGE";
	public static final String BLUE= "BLUE";
	public static final String GREY= "GREY";
	public static final String COLOR_IS_ACTIVE= "Y";
	public static final Integer ZERO= 0;
	public static final String ACTIVITY_COMPLETE= "2";
	public static final String AUTH_COMPANY_ID="authCompanyId";
	public static final String AUTH_USER_ID="authUserId";
	public static final String MEMBER_LABEL_KEY="MBR";
	public static final String SINAPSI_MONITOR_PRIORITY="SMNT_PRT";
	public static final String SINAPSI_MONITOR_DEPARTMENT="SMNT_DPT";
	public static final String RESOURCE_TYPE="M";

	public static final String CONFIRMED = "1";
	public static final String IN_PROGRESS = "3";
	public static final int DIFF_DATE=4;
	public static final String DEFAULT_GROUP="P10";
	public static final String ACCOUNT_GEONAME_WS="dimitridevelopper";
	public static final String LOGIN_REDIRECT="loginAction";
	public static final String CURRENT_USER="CURRENT_USER";
	public static final String CURRENT_ASS="CURRENT_ASS";
	public static final String  LOCALE="LOCALE";
	public static final String ADMINISTRATOR="admin";
	public static final String ADMIN_SESSION="ADMIN";
	public static final String  MEMBER_SESSION="MEMBER";
	public static final String  USR_SESSION="USR";
	public static final String  PASS_SESSION="PASS";
	public static final String  ASS_SESSION="ASS";
	public static final String  REM_SESSION="REM";


}
